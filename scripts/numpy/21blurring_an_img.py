...
# Note that there is an entire SciPy subpackage, scipy.ndimage, devoted to image
# processing. This example serves simply to illustrate the syntax and format
# of NumPy's two-dimensional FFT implementation.

# The two-dimensional DFT is widely-used in image processing. For example,
# multiplying the DFT of an image by a two-dimensional Gaussian function is a
# common way to blur an image by decreasing the magnitude of its high-frequency
# components.

# The following code produces an image of randomly-arranged squares and then
# blurs it with a Gaussian filter.
import numpy as np
import pylab
